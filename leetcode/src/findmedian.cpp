...
private:
    ...
    double find(int *A, int n, int *B, int m, int th){ // th is 0-based
        ...
        if(mida + 1 + midb + 1 <= th + 1){ // both pre halves sum count is less than total mid
            int skipped = midb + 1; // skip pre half of B[], including B[midb]
            return find(A, n, B + skipped, m - skipped, th - skipped);
        }else{  // total mid(th) shoild be in sum set of both pre halves, skip latter half of A[] which includes A[mida]
