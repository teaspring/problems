...
bool assertBinaryTree(TreeNode *p1, TreeNode *p2){
    if(!p1 || !p2){
        EXPECT_TRUE(!p1 && !p2);
        return true;
    }
    ...
    EXPECT_TRUE(assertBinaryTree(p1->left, p2->left));
    EXPECT_TRUE(assertBinaryTree(p1->right, p2->right));
    return true;
}
...
TEST(largestBSTInBT_Test, Positive01){
    ...
    EXPECT_TRUE(assertBinaryTree(expected, result));
